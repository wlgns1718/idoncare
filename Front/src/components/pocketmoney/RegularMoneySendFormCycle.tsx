...
interface handleFirstOptionChangeProps {
...
function RegularMoneySendFormCycle() {
  ...
  const handleFirstOptionChange = ({ selectedOption }: handleFirstOptionChangeProps) => {
    setFirstOption(selectedOption);
  };

  const filteredDayOptions =
    firstOption === ""
      ? []
      : firstOption === "daily"
      ? [{ value: "Daily", label: "Daily" }]
      : firstOption === "weekly"
      ? dayOptions.slice(0, 7)
      : dayOptions.slice(7);
